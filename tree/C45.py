...
class C45Classifier:
    ...
    def __calculate_entropy(self, data, weights):
        ...
        for count in class_counts.values():
            probability = (
                count / total_weight
            )  # Menghitung probabilitas masing-masing label kelas
            entropy -= probability * math.log2(
                probability
            )  # Menghitung kontribusi entropi dari masing-masing label
    ...
    def __split_data(self, data, attribute_index, attribute_value, weights):
        ...
        for i, record in enumerate(data):
            if record[attribute_index] == attribute_value:
                split_data.append(
                    record[:attribute_index] + record[attribute_index + 1 :]
                )
    ...
    def __select_best_attribute_c50(self, data, attributes, weights):
        ...
        for attribute_index in range(len(attributes)):
            ...
            for value in attribute_values:
                subset, subset_weights = self.__split_data(
                    data, attribute_index, value, weights
                )
    ...
    def __train(self, data, weight=1):
        ...
        self.attributes = data.columns.tolist()[
            :-1
        ]  # Mendapatkan atribut dari kolom dataset
        ...
        if attribute_values in tree.children:
            ...
            if len(class_labels) == 0:
                return self.__majority_class(
                    self.data.values.tolist(), [1.0] * len(self.data)
                )
