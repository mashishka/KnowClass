    ...
    def Consulting(self, *args):
        ...
        while flag:
            if type(tree_dict) != dict:  # если консультация закончилась:
                ...
                self._comboboxes[-1].currentIndexChanged.connect(self.onCurrentIndexChanged)
                answer = self._ans
                log.debug(f"ответ получен: {answer}")

                i = answers.index(answer)
                if type(tree_dict[question]["children"][i][answer]["children"][0]) == dict:
                    tree_dict = tree_dict[question]["children"][i][answer]["children"][0]
                else:
