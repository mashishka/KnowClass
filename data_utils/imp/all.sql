--
-- File generated with SQLiteStudio v3.4.4 on —р май 8 22:54:06 2024
--
...
CREATE TABLE IF NOT EXISTS AdditionalData (id INTEGER PRIMARY KEY NOT NULL, result_name TEXT NOT NULL DEFAULT RESULT, result_text TEXT NOT NULL DEFAULT "", result_type TEXT CHECK (result_type IN ("probability", "confidence")), tree_data BLOB, example_positions TEXT NOT NULL DEFAULT "[]", factor_positions TEXT NOT NULL DEFAULT "[]", result_value_positions TEXT NOT NULL DEFAULT "[]");
INSERT INTO AdditionalData (id, result_name, result_text, result_type, tree_data, example_positions, factor_positions, result_value_positions) VALUES (1, 'RESULT', '', NULL, NULL, '[]', '[]', '[]');
...
-- Table: ResultValue
CREATE TABLE IF NOT EXISTS ResultValue (result_value_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name TEXT NOT NULL UNIQUE, text TEXT NOT NULL DEFAULT "", likelihood REAL);
...
CREATE TRIGGER IF NOT EXISTS result_value_position_delete AFTER DELETE ON ResultValue FOR EACH ROW BEGIN UPDATE AdditionalData
SET result_value_positions = json_remove(result_value_positions, '$['||
(SELECT json_each.id - 1
FROM json_each((SELECT result_value_positions FROM AdditionalData))
WHERE json_each.value = old.result_value_id)
||"]")
WHERE id = 1; END;

-- Trigger: result_value_position_insert
CREATE TRIGGER IF NOT EXISTS result_value_position_insert AFTER INSERT ON ResultValue FOR EACH ROW BEGIN UPDATE AdditionalData
